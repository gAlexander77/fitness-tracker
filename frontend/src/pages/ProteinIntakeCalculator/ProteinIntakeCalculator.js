...
function Calculator(props) {
  ...
  const calculateHandler = () => {
    ...
    let proteinIntake = calculateProtein(height, props.input.weight, currentGender, props.input.activityLevel, props.input.age);

    props.setResults({ proteinIntake: proteinIntake });
    if (
      props.results.proteinIntake > 0 &&
      props.input.feet !== 0 &&
  ...
  return (
    <div className="protein-intake-calculator-calculator-component">
      ...
      <div className='protein-intake-calculator-gender-btns'>
            <button
                id='male'
                className={ `protein-intake-calculator-gender-btn ${currentGender === 5 ? 'active' : ''}`}
                onClick={() => handleGenderButtonClick(5)}>
            M        
        </button>
            <button
                id='female'
                className={`protein-intake-calculator-gender-btn ${currentGender === -161 ? 'active' : ''}`}
